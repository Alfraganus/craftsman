...
class Content extends Contents
{
    ...
    public static function getItems($page_type = '', $args = array())
    {
        ...
        $test = self::$selected_language = array_value($current_lexicon, 'lang_code', 'en');
        ...
        $query = self::find()
            ...
            ->where(['content.type' => self::$type, 'info.language' => 'ru']);
    ...
    public static function getPageTypes($active_key = '')
    {
        $page_types = array(
            '' => array(
                'name' => 'All',
                'active' => false,
                'count' => self::allCount(['content.type' => self::$type, 'content.deleted' => 0]),
            ),
            'published' => array(
                'name' => 'Published',
                'active' => false,
                'count' => self::publishedCount(['content.type' => self::$type]),
            ),
            'unpublished' => array(
                'name' => 'Unpublished',
                'active' => false,
                'count' => self::unpublishedCount(['content.type' => self::$type]),
            ),
            'deleted' => array(
                'name' => 'Deleted',
                'active' => false,
                'count' => self::deletedCount(['content.type' => self::$type]),
            ),
        );
    ...
    public static function publishedCount($where = array())
    {
        $query = self::find()
            ->alias('content')
            ->join('INNER JOIN', 'site_content_info info', 'info.content_id = content.id')
            ->where(['content.deleted' => 0, 'content.status' => 1]);
    ...
    public static function unpublishedCount($where = array())
    {
        $query = self::find()
            ->alias('content')
            ->join('INNER JOIN', 'site_content_info info', 'info.content_id = content.id')
            ->where(['content.deleted' => 0, 'content.status' => 0]);
